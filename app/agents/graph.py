...
from app.agents.nodes.tools import ToolsNode
...
class MedicalRAGGraph:
    ...
    def _build_graph(self) -> StateGraph:
        ...
        START → Translate → Router → Synthesize → Judge → END
        ...
        workflow.add_node("tools", self._tools_step)
        ...
        workflow.add_conditional_edges("router", self.choose_path, {
            ...
            "nedrex": "tools",
            "both": "tools",
            "chitchat": "chat"  # If it's chitchat, skip retrieval and go to chat node
        })
    ...
    def _judge_step(self, state: AgentState) -> AgentState:
        ...
        return state
    def _tools_step(self, state: AgentState) -> AgentState:
        """Run tools node to retrieve information."""
        logger.info("Running TOOLS step")
        state = self.tools.use_tools(state)
        state.setdefault("steps_completed", []).append("tools")
        return state
